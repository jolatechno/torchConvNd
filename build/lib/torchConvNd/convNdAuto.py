...
def convNdRecAuto(input, mem, output_shape, func, kernel, dilation=1, padding_mode='constant', padding_value=0, *args):
	...
	stride, padding = autoStridePad(list(input.shape), output_shape[0], kernel[0], dilation[0])
	mem_stride, mem_padding = autoStridePad(list(mem.shape), output_shape[1], kernel[1], dilation[1])
