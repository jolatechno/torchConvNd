...
def convNdRecAuto(input, mem, output_shape, func, kernel, dilation=1, padding_mode='constant', padding_value=0, *args):
	...
	kernel, dilation = sequencify(kernel, 2, in_dim), sequencify(dilation, 2, in_dim)

	stride, padding = autoStridePad(list(input.shape), output_shape, kernel[0], dilation[0])
	mem_stride, mem_padding = autoStridePad(list(mem.shape), list(mem.shape), kernel[1], dilation[1])
	...
	if mem_result.ndim != in_dim:
		mem_result = convPost(mem_result, mem_shape)

	for i in range(input.ndim):
		result = result.narrow(i, 0, output_shape[i])
		mem_result = mem_result.narrow(i, 0, mem.shape[i])
